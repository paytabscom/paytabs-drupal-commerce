...
class Paytabs_core2
{
  ...
  {
    $var = trim($var);
    $var = \Drupal\commerce_paytabs_pt2\PaytabsHelper::getNonEmpty($var, ...$options);
  }
 ...
 * Holder class that holds PayTabs's request's values
 */
class PaytabsTransactionOperationsHolder
{
   ...
   * @return array
   */
  public function pt_build($operation_type)
  {
    $all = array_merge(
      [
        'tran_type' => $this::OPERATION_TYPES[$operation_type],
        'tran_class' => 'ecom'
      ],
      $this->operationInfo,
      $this->transaction_id
  ...
  public function set01OperationInfo($amount, $cart_currency)
  {
    $this->operationInfo = [
      'cart_amount' => (float)$amount,
...
class PaytabsApi
{
  ...
  public function __construct($region, $profile_id, $server_key)
  {
